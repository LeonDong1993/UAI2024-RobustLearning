...
def create_graph(data, max_parents = -1, corr_thresh = 0.05):
    D = data.shape[1]
    max_parents = max_parents if max_parents > 0 else D
    
    G = Graph(digraph = False).make_complete_graph(D)
    corrs = analyze_correlation(data)
    ...
    for i in range(D):
        ...
        for k,j in enumerate(rank[1:]):
            p = par[j]
            if k >= max_parents or corrs[p,j] < corr_thresh:
                G.remove_edge(p, i)
    G.enhance()
    return G
...
class PGNN(nn.Module):
    ...
    @staticmethod
    def weighted_loss_func(nn_out, ydata, G, R = None, return_mass = False):
        # R is the adversial weights for each input data point
        # should be of same size as ydata.shape[0]
        # R need to be a tensor array, it is not a parameter
        ...
        log_mass = -0.5 * ( torch.log(2 * np.pi / P) + torch.square(ydata - mean) * P )
        sample_log_mass = torch.sum(log_mass, dim = 1)

        if return_mass:
            return sample_log_mass

        loss = - torch.sum (sample_log_mass * R ) / ydata.shape[0]
        return loss
